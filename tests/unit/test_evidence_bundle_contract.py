...
def _load_bundle(tmp_path: Path, facade: object) -> dict[str, object]:
    """Run a falsification slice and return the parsed bundle dict."""
...
def test_world_manifest_contains_track_aliases(tmp_path: Path) -> None:
    """Test that world_manifest contains both track and track_id aliases."""
    facade = create_facade()
    payload = _load_bundle(tmp_path, facade)
    manifest = payload["world_manifest"]

    assert "track" in manifest, "world_manifest must contain 'track' alias"
    assert "track_id" in manifest, "world_manifest must preserve 'track_id'"
    assert manifest["track"] == "suzuka"
    assert manifest["track_id"] == "suzuka"
    assert manifest["track"] == manifest["track_id"]
...
def test_replay_integrity_declares_partial_hashing(tmp_path: Path) -> None:
    ...
    pending_note_found = any(
        "full state snapshot hashing pending" in note
        for note in replay_integrity["notes"]
    )
    assert pending_note_found, (
