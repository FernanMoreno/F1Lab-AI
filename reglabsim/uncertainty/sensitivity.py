...
class SensitivityAnalyzer:
    ...
    def analyze(
        ...
        samples = {
            name: rng.uniform(low, high, n_samples)
    ...
    def sobol_indices(
        ...
        base_samples = {
            name: rng.uniform(low, high, n)
            for name, (low, high) in params.items()
        }

        # Evaluate at base and perturbed points
        base_output = np.array([sim_fn({k: v[i] for k, v in base_samples.items()}) for i in range(n)])
        ...
        for name in params.keys():
            ...
            perturbed_output = np.array([sim_fn({k: v[i] for k, v in perturbed_samples.items()}) for i in range(n)])
